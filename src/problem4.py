...
def problem4(point1, point2, n, window):
    ...
    window.render()
    x_val = (point2.x - point1.x)/(2*n)
    y_val = (point2.y - point1.y)/(2*n)
    for k in range(n):
        new_point = rg.Point(x_val, y_val)
        new_point.attach_to(window)
        x_val = x_val + (point2.x - point1.x)/(2*n)
        y_val = y_val + (point2.y - point1.y)/(2*n)
        window.render()
